...
// The inverse CDF function. The parameter "model" is used in interpolation.
{
    if (invCDFTab->isMappingTableLoaded()) {
        ...
        cout << "RandomVariable::invCDF error: no mapping table loaded."
             << endl;
        exit(-1);
    }
}
...
// The envelop pdf function. The parameter "model" is used in interpolation.
{
    if (envelopPdfTab->isMappingTableLoaded()) {
        ...
        cout << "RandomVariable::envelopPdfTab error: no mapping table loaded."
             << endl;
        exit(-1);
    }
}
...
// The envelop inverse CDF function.
{
    if (envelopInvCDFTab->isMappingTableLoaded()) {
        ...
        cout << "RandomVariable::envelopInvCDF error: no mapping table loaded."
             << endl;
        exit(-1);
    }
}
...
//----------------------------------------------------------------------
double RandomVariable::sampleUsingPDFDirect(
    double normalization_factor, double LB, double RB)
...
// -- LB, RB: only numbers in [LB,RB] are sampled
{
    ...
    while (1) {
        ...
        if (numberOfIteration > MAXITER) {
            cout << "RandomVariable::sampleUsingPDFDirect warning: max number "
                    "of iteration MAXITER reached."
                 << endl;
            return x;
        }
    }
...
{
    return sampleUsingPDFDirect(
        normalization_factor, pdfTab->getXMin(), pdfTab->getXMax());
}

//----------------------------------------------------------------------
double RandomVariable::sampleUsingPDFAndEnvelopFunc(
    double normalization_factor, double LB, double RB)
...
// -- LB, RB: only numbers in [LB,RB] are sampled
{
    ...
    while (1) {
        ...
        if (drand48() < pdf(x) / (normalization_factor * envelopPdf(x) + 1e-60))
            return x;
        numberOfIteration++;
        if (numberOfIteration > MAXITER) {
            cout << "RandomVariable::sampleUsingPDFAndEnvelop warning: max "
                    "number of iteration MAXITER reached."
                 << endl;
            return x;
        }
    }
...
// Short version, only usable if envelopInvCDFTab and envelopPdfTab are loaded.
{
    return sampleUsingPDFAndEnvelopFunc(
        normalization_factor, envelopInvCDFTab->getXMin(),
        envelopInvCDFTab->getXMax());
}
...
// \int x^order pdf(x) dx
{
    ...
    return qiu_simpsons(
        &zq_moments_integrad_hook, LB, RB, EPSILON, MAXRECURSION);
}

//----------------------------------------------------------------------
void RandomVariable::constructEnvelopTab(
    double center, double width, int step_left, int step_right)
// This function construct envelop functions tables using step functions.
// It scan the range from "center - step_left*width" to "center +
// step_right*width". For each interval of size "width", it assumes that the pdf
// is monotonic and the larger of the values at the two ends will be used as the
// value of the envelop pdf in this region, from which envelop CDF will be
// calculated.
...
// table is used by functions like sampleUsingPDFAndEnvelopTab.
{
    ...
    double RB = LB + width;
    double pdfLB = pdf(LB), pdfRB = pdf(RB),
           pdfLarger;  // x and y values at the two ends
    int idx = 1;
    // initial values
    ...
    envelopPdfTab->interpolation_model =
        10;  // nearest neighbor direct interpolation
    envelopInvCDFTab->interpolation_model = 2;  // linear mono
}

//----------------------------------------------------------------------
void RandomVariable::calculateInvCDFFromPdf(
    double LB, double RB, long number_of_points)
