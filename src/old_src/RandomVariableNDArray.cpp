...
RandomVariableNDArray::RandomVariableNDArray(
    vector<double>* data_in, vector<long>& sizes_in, double zero,
    bool destructive_in)
...
// to be the inverse CDF; otherwise a new memory space will be allocated.
{
    ...
    data_sum = 0.0;
    for (long current_index = 1;
         current_index <= (*product_of_sizes)[dimension - 1]; current_index++) {
...
{
    long l = binarySearch(
        invCDF, (*invCDF)[0] + (data_sum - (*invCDF)[0]) * drand48());
