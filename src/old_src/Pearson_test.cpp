...
int main() {
    ...
    // nbd.envelopPdfTab->map(i) << endl; for (double i=0; i<0.1; i+=0.003641)
    // cout << setw(10) << i << "  " << nbd.envelopInvCDFTab->map(i) << endl;
    ...
    for (long i = 0; i < n_sample; i++) {
        ...
        int idx =
            static_cast<int>((random_sample + (hist_size + dx) / 2.) / dx);
    ...
    for (int i = 0; i < hist_length; i++) {
        checkof << scientific << setw(18) << setprecision(6) << hist_x[i]
                << "  " << test.pdf(hist_x[i]) * norm << "  " << hist[i]
                << endl;
    }
