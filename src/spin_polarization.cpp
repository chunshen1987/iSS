...
SpinPolarization::SpinPolarization(
    ...
    const std::vector<particle_info> &particles, std::string path,
    std::string table_path, ParameterReader &paraRdr)
    : path_(path),
      table_path_(table_path),
      FOsurf_ptr_(FOsurf_ptr),
      particle_info_(particles),
      paraRdr_(paraRdr) {
    double dpT = 3.0 / NpT_;
    pT_arr_.resize(NpT_);
    for (int i = 0; i < NpT_; i++) pT_arr_[i] = (i + 0.5) * dpT;
    ...
    y_arr_.resize(Ny_);
    for (int i = 0; i < Ny_; i++) y_arr_[i] = -y_size / 2. + i * dy;
...
#pragma omp parallel for
    for (int i = 0; i < 2; i++) {
        if (i == 0) {
            cout << "OpenMP: using " << omp_get_num_threads() << " threads."
                 << endl;
        }
    }
    ...
    int PolRapType =
        static_cast<int>(paraRdr_.getVal("polarizationRapType", 2));
...
void SpinPolarization::compute_spin_polarization(
    ...
    for (int iy = 0; iy < Ny_; iy++) {
        ...
        for (int ipT = 0; ipT < NpT_; ipT++) {
            ...
            for (int iphi = 0; iphi < Nphi_; iphi++) {
                ...
                iSS_data::Vec4 pmu = {
                    static_cast<float>(p0), static_cast<float>(px),
                ...
                compute_spin_polarization_for_a_given_p(
                    POI_info, pmu, ivor_type, dN, Smu, Smu_LRF, SmuSIP1,
                    SmuSIP1_LRF, SmuSIP2, SmuSIP2_LRF, SmuFull, SmuFull_LRF);
    ...
    compute_integrated_spin_polarizations(
        Smu_pTdpTdphidy_, SmuLRF_pTdpTdphidy_);
...
void SpinPolarization::output_integrated_spin_polarizations(
    ...
    const std::string thermal_shear_typenames, const int Flag_MuIP,
    const int Flag_SIP) {
    cout << "output spin polarization results to files ... " << endl;
    std::ofstream of;
    std::stringstream fileTypeName;

    fileTypeName << vorticity_typename << "_" << rap_typename << "_"
                 << POI_monval;
    if (Flag_MuIP == 1) fileTypeName << "_" << "wMuIP";
    if (Flag_SIP == 1) fileTypeName << "_wSIP_" << thermal_shear_typenames;
    ...
    for (int i = 0; i < 4; i++) {
        of << scientific << setw(10) << setprecision(6) << SmuLRF_[i] << "  ";
    }
    of << endl;
    of.close();
    ...
    for (int ipT = 0; ipT < NpT_; ipT++) {
        of << scientific << setw(10) << setprecision(6) << pT_arr_[ipT] << "  "
           << dN_pT_[ipT] << "  ";
        for (int i = 0; i < 4; i++) {
            of << scientific << setw(10) << setprecision(6) << Smu_pT_[ipT][i]
               << "  ";
        }
        for (int i = 0; i < 4; i++) {
    ...
    for (int iphi = 0; iphi < Nphi_; iphi++) {
        of << scientific << setw(10) << setprecision(6) << phi_arr_[iphi]
           << "  " << dN_phi_[iphi] << "  ";
        for (int i = 0; i < 4; i++) {
            of << scientific << setw(10) << setprecision(6) << Smu_phi_[iphi][i]
               << "  ";
        }
        for (int i = 0; i < 4; i++) {
            of << scientific << setw(10) << setprecision(6)
    ...
    for (int iy = 0; iy < Ny_; iy++) {
        of << scientific << setw(10) << setprecision(6) << y_arr_[iy] << "  "
           << dN_y_[iy] << "  ";
        for (int i = 0; i < 4; i++) {
            of << scientific << setw(10) << setprecision(6) << Smu_y_[iy][i]
               << "  ";
        }
        for (int i = 0; i < 4; i++) {
            of << scientific << setw(10) << setprecision(6) << SmuLRF_y_[iy][i]
               << "  ";
        }
        of << endl;
    }
    ...
    for (int ipT = 0; ipT < NpT_; ipT++) {
        for (int iphi = 0; iphi < Nphi_; iphi++) {
            of << scientific << setw(10) << setprecision(6) << pT_arr_[ipT]
               << "  " << phi_arr_[iphi] << "  " << dN_pTdpTdphi_[ipT][iphi]
               << "  ";
            for (int i = 0; i < 4; i++) {
                of << scientific << setw(10) << setprecision(6)
    ...
    // Smu_dpTdphidy_filename << path_ << "/Smu_dpTdphidy_" <<
    // fileTypeName.str()
    //                        << ".dat";
...
void SpinPolarization::compute_spin_polarization_for_a_given_p(
    ...
    const int ivor_type, double &dN, iSS_data::Vec4 &Smu,
    iSS_data::Vec4 &SmuLRF, iSS_data::Vec4 &SmuSIP1, iSS_data::Vec4 &SmuSIP1LRF,
...
#pragma omp parallel for reduction(                              \
        + : Smu_tmp[ : 4], SmuSIP1_tmp[ : 4], SmuSIP2_tmp[ : 4], \
            SmuFull_tmp[ : 4], dN)
    for (unsigned int i = 0; i < FOsurf_ptr_.size(); i++) {
        ...
        const float tau_peta =
            pmu[3] * surf.cosh_eta - pmu[0] * surf.sinh_eta;  // tau*p^eta
        const double mu =
            (POI_info.baryon * surf.muB + POI_info.strange * surf.muS
             + POI_info.charge * surf.muQ);
        const double pdotu =
            pmu[0] * ut - pmu[1] * ux - pmu[2] * uy - pmu[3] * uz;
        const double pdsigma =
            (tau
             * (ptau * surf.da0 + pmu[1] * surf.da1 + pmu[2] * surf.da2
                + tau_peta * surf.da3 / tau)
             / hbarC3);  // [GeV^-2]
        ...
        float sigma[10];
        for (int j = 0; j < 10; j++) sigma[j] = surf.vorticity_arr[24 + j];
        ...
        // Becattini
        pdotsigma[0] =
            (pmu[0] * sigma[0] - pmu[1] * sigma[1] - pmu[2] * sigma[2]
             - pmu[3] * sigma[3]);
        pdotsigma[1] =
            (pmu[0] * sigma[1] - pmu[1] * sigma[4] - pmu[2] * sigma[5]
             - pmu[3] * sigma[6]);
        pdotsigma[2] =
            (pmu[0] * sigma[2] - pmu[1] * sigma[5] - pmu[2] * sigma[7]
             - pmu[3] * sigma[8]);
        pdotsigma[3] =
            (pmu[0] * sigma[3] - pmu[1] * sigma[6] - pmu[2] * sigma[8]
             - pmu[3] * sigma[9]);
        Smu_SIP1[0] = 0.;
        Smu_SIP1[1] =
            prefactorSIP * (-pmu[2] * pdotsigma[3] + pmu[3] * pdotsigma[2]);
        Smu_SIP1[2] =
            prefactorSIP * (pmu[1] * pdotsigma[3] - pmu[3] * pdotsigma[1]);
        Smu_SIP1[3] =
            prefactorSIP * (-pmu[1] * pdotsigma[2] + pmu[2] * pdotsigma[1]);
        ...
        pdotsigma[0] =
            (p_perpMu[0] * sigma[0] - p_perpMu[1] * sigma[1]
             - p_perpMu[2] * sigma[2] - p_perpMu[3] * sigma[3]);
        pdotsigma[1] =
            (p_perpMu[0] * sigma[1] - p_perpMu[1] * sigma[4]
             - p_perpMu[2] * sigma[5] - p_perpMu[3] * sigma[6]);
        pdotsigma[2] =
            (p_perpMu[0] * sigma[2] - p_perpMu[1] * sigma[5]
             - p_perpMu[2] * sigma[7] - p_perpMu[3] * sigma[8]);
        pdotsigma[3] =
            (p_perpMu[0] * sigma[3] - p_perpMu[1] * sigma[6]
        ...
        float DmuB_over_T[4];
        for (int j = 0; j < 4; j++) DmuB_over_T[j] = surf.vorticity_arr[34 + j];
        const double prefactorMuIP =
            prefactor * hbarC * POI_info.baryon / pdotu;
...
void SpinPolarization::transformSmuToLRF(
    const double mass, const iSS_data::Vec4 &pmu, double Smu[],
    iSS_data::Vec4 &SmuLRF) const {
    // transform to the particle LRF
    double p_dot_S = 0.;
    for (int i = 1; i < 4; i++) {
        p_dot_S += pmu[i] * Smu[i];
