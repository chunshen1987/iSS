...
void TestBosonSampler(
    double m, double T, double mu,
    std::shared_ptr<RandomUtil::Random> ran_gen_ptr);

int main() {
    int randomSeed = -1;
    auto ran_gen_ptr =
        std::shared_ptr<RandomUtil::Random>(new RandomUtil::Random(randomSeed));
    std::vector<double> mass_list = {0.138, 0.49, 1.0, 1.5, 2.0};
    std::vector<double> T_list = {0.05, 0.1, 0.15, 0.2};
    std::vector<double> mu_list = {-0.9, -0.5, -0.3, -0.1, 0.0,
                                   0.1,  0.3,  0.5,  0.9};
...
void TestBosonSampler(
    ...
    for (unsigned int i = 0; i < hist_x.size(); i++) {
        of << std::scientific << std::setprecision(6) << hist_x[i] << "  "
           << pdf[i] << "  " << hist_y[i] << "  " << hist_y_err[i] << std::endl;
    }
    of.close();
