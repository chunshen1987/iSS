...
EmissionFunctionArray::EmissionFunctionArray(
    ...
    Table *pt_tab_in, Table *phi_tab_in, Table *y_minus_eta_tab_in,
    ...
    // or phi_tab4Sampling in terms of pT_tab or phi_tab; indecies starts
    // with 1.
...
// needed to calcualte flows.
{
    ...
    double prefactor =
        1.0 / (8.0 * (M_PI * M_PI * M_PI)) / hbarC / hbarC / hbarC;
    ...
    for (int i = 0; i < pT_tab_length; i++) {
        ...
        mT_tab_double[i] =
            sqrt(mass * mass + pT_tab_double[i] * pT_tab_double[i]);
    }
        ...
        for (int k = 0; k < y_minus_eta_tab_length; k++) {
            ...
            for (int i = 0; i < pT_tab_length; i++) {
                ...
                for (int j = 0; j < phi_tab_length; j++) {
                    ...
                    double pdotu =
                        pt * gammaT - px * ux - py * uy - pz * tau_ueta;
                    double expon = (pdotu - mu) / Tdec;
                    // thermal equilibrium distributions
                    double f0 = 1. / (exp(expon) + sign);

                    double pdsigma =
                        pt * da0 + px * da1 + py * da2 + pz * da3 / tau;

                    // viscous corrections
                    double delta_f_shear = 0.0;
                    if (INCLUDE_DELTAF) {
                        double Wfactor =
                            (pt * pt * pi00 - 2.0 * pt * px * pi01
                             - 2.0 * pt * py * pi02 - 2.0 * pt * pz * pi03
                             + px * px * pi11 + 2.0 * px * py * pi12
                             + 2.0 * px * pz * pi13 + py * py * pi22
                             + 2.0 * py * pz * pi23 + pz * pz * pi33);
                        delta_f_shear =
                            (1 - sign * f0) * Wfactor * deltaf_prefactor;
                    }
                    ...
                    if (INCLUDE_DIFFUSION_DELTAF == 1) {
                        ...
                        delta_f_qmu =
                            ((1. - sign * f0) * (prefactor_qmu - baryon / pdotu)
    ...
    sw.toc();
    messager << " -- Calculate_dN_dxtdetady finished in " << sw.takeTime()
             << " seconds.";
    messager.flush("info");
}
//***************************************************************************
...
void EmissionFunctionArray::calculate_dN_pTdpTdphidy(int particle_idx) {
    ...
    // Note that the execution fo this function does NOT provide the
    // dN_dxtdetady array which is needed for sampling.
    last_particle_idx = particle_idx;
    Stopwatch sw;
    sw.tic();
    ...
    for (int i = 0; i < pT_tab_length; i++) {
        ...
        for (int j = 0; j < phi_tab_length; j++) {
            ...
            for (long l = 0; l < FO_length; l++) {
                ...
                double mu = baryon * surf->muB + strange * surf->muS
                            + charge * surf->muQ;
                if (flag_PCE_ == 1) {
                    double mu_PCE = surf->particle_mu_PCE[particle_idx];
                    mu += mu_PCE;
    ...
    sw.toc();
    messager << " -- Calculate_dN_pTdpTdphidy finished in " << sw.takeTime()
             << " seconds.";
    messager.flush("info");
}
//***************************************************************************
...
// Calculate flow from order from_order to to_order and store them to files.
{
    ...
    // for (int i=0; i<1; i++) // for debugging
    {
        ...
        {
            double phi = phi_tab->get(1, j + 1),
                   phi_weight = phi_tab->get(2, j + 1);
            double dN = dN_pTdpTdphidy->get(i + 1, j + 1);

            normalization[i] += dN * phi_weight;
            for (int order = from_order; order <= to_order; order++) {
                vn[i][order - from_order][0] +=
                    dN * phi_weight * cos(order * phi);
                vn[i][order - from_order][1] +=
                    dN * phi_weight * sin(order * phi);
            }
        }
    ...
    // for (int i=0; i<1; i++) // for debugging
    {
        ...
        for (int order = from_order; order <= to_order; order++) {
            vni[order - from_order][0] +=
                vn[i][order - from_order][0] * pT * pT_weight;
            vni[order - from_order][1] +=
                vn[i][order - from_order][1] * pT * pT_weight;
        }
    }
    ...
    for (int t = 0; t < number_of_flows; t++) {
        ...
        vn_inte.set(
            6, t + 2,
            sqrt(
                vni[from_order + t - 1][0] * vni[from_order + t - 1][0]
    ...
    sw.toc();
    // cout << "calculate_flows finishes " << sw.takeTime() << " seconds." <<
    // endl;
}
//***************************************************************************
...
// If perform_sampling is 1 then sample_using_dN_pTdpTdphidy will be called.
{
    ...
    for (int n = 0; n < Nparticles; n++) {
        ...
        ofstream of1(
            flow_differential_filename_old.c_str(), std::ios_base::app);
        ...
        of2.close();
        calculate_flows(
            to_order, flow_differential_filename_old,
...
// but flows in separated ones.
{
    ...
    Table zero(
        dN_pTdpTdphidy->getNumberOfCols(), dN_pTdpTdphidy->getNumberOfRows(),
        0);
    for (int n = 0; n < Nparticles; n++) {
        if (dNs[n] == NULL) {
            zero.printTable(of);
...
void EmissionFunctionArray::sample_using_dN_dxtdetady_smooth_pT_phi() {
    ...
    RandomVariable2DArray rand2D(
        dN_dxtdetady_with_weight, FO_length, y_minus_eta_tab_length,
        ran_gen_ptr, zero);
    ...
    samples_control_filename << path_ << "/samples_control_" << particle->monval
                             << ".dat";
    ...
    for (long sampling_idx = 1; sampling_idx <= number_of_repeated_sampling;
        ...
        while (sample_idx <= number_to_sample) {
            ...
            double y_minus_eta_s =
                y_minus_eta_tab->get(1, y_minus_eta_s_idx + 1);
            ...
            while (tries < maximum_impatience) {
                ...
                double delta_f_bulk = get_deltaf_bulk(
                    mass, pdotu, bulkPi, Tdec, sign, f0, bulkvisCoefficients);
                ...
                if (flag_restrict_deltaf == 1) {
                    ...
                    double ratio_max = deltaf_max_ratio;
                    double deltaf_size =
                        std::abs(delta_f_shear + delta_f_bulk + delta_f_qmu);
                    resize_factor =
                        (std::min(1., ratio_max / (deltaf_size + 1e-10)));
                }
                result =
            ...
            if (positive_y_minus_eta_table_only) {
                ...
                if (flip_a_coin > 0.5) y_minus_eta_s = -y_minus_eta_s;
            }
            ...
            if (!USE_OSCAR_FORMAT) {
                snprintf(
                    line_buffer, 500,
                    "%lu  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  "
                    "%e  %e  %e  %e\n",
                    FO_idx, surf->tau, surf->xpt, surf->ypt, y_minus_eta_s, pT,
                    phi, surf->da0, surf->da1, surf->da2, vx, vy, y, eta_s, E,
                    p_z, t, z);
            }
        ...
        if (AMOUNT_OF_OUTPUT > 0)
            print_progressbar(
                static_cast<double>(sampling_idx)
                / number_of_repeated_sampling);
    }
    // flushing buffers
    ...
    if (!USE_OSCAR_FORMAT) {
        of_sample_format << "Total_number_of_columns = " << 18 << endl
                         << "FZ_cell_idx = " << 1 << endl
                         << "tau = " << 2 << endl
                         << "FZ_x = " << 3 << endl
                         << "FZ_y = " << 4 << endl
    ...
    if (USE_OSCAR_FORMAT) {
        of_sample_format << "Total_number_of_columns = " << 9 << endl
                         << "t = " << 9 << endl
                         << "FZ_x = " << 6 << endl
                         << "FZ_y = " << 7 << endl
                         << "z = " << 8 << endl
...
void EmissionFunctionArray::sample_using_dN_pTdpTdphidy() {
    // Sample according to the dN_pTdpTdphidy and dN_pTdpTdphidy_max array.
    // These arrays are asssumed to be already cacluated. The particle index is
    // assumed to be already stored in the "last_particle_idx" internal
    // variable. Emission function below "zero" will be treated as "zero". Each
    // line in the sample file contains the following data: FZ_cell_idx tau,
    // xpt, ypt, y_minus_eta_s, pT, phi, da0, da1, da2, vx, vy Each line in the
    // sample control file records the number of particles sampled in one event.
    // The format file is a file that can be read by ParameterReader class
    // which records the the purpose of each column in the sample file. For
    // example, if one line of it reads as "foo=1" means that the variable
    // "foo" is the recorded in the 1st column.
    ...
    for (int i = 0; i < pT_tab4Sampling_length; i++) {
        for (int j = 0; j < phi_tab4Sampling_length; j++) {
            ...
            double phi_weight =
                phi_tab4Sampling.get(2, j + 1);  // get phi weight
            double phi_index = phi_tab4Sampling.get(3, j + 1);  // get phi index
            dN_pTdpTdphidy_with_weight_4Sampling[i][j] =
                (dN_pTdpTdphidy->interp2(pT_index, phi_index, 4) * pT
                 * pT_weight * phi_weight);
    ...
    RandomVariable2DArray rand2D(
        dN_pTdpTdphidy_with_weight_4Sampling, phi_tab4Sampling_length,
        pT_tab4Sampling_length, ran_gen_ptr, zero);
    ...
    for (int i = 0; i < pT_tab_length; i++) {
        for (int j = 0; j < phi_tab_length; j++) {
            dN_dy_CF +=
                (dN_pTdpTdphidy->get(i + 1, j + 1) * pT_tab->get(1, i + 1)
                 * pT_tab->get(2, i + 1) * phi_tab->get(2, j + 1));
        }
    }
    ...
    for (long i = 0; i < pT_tab4Sampling_length; i++)
        mT_tab[i] = sqrt(
            mass * mass
            + pT_tab4Sampling.get(1, i + 1) * pT_tab4Sampling.get(1, i + 1));
    ...
    for (long sampling_idx = 1; sampling_idx <= number_of_repeated_sampling;
        ...
        while (sample_idx <= number_to_sample) {
            ...
            while (tries < maximum_impatience) {
                ...
                // get a surface index
                FO_idx =
                    floor(ran_gen_ptr->rand_uniform() * (FO_length - 1e-30));
                ...
                double pdotu_max =
                    pt_max * gammaT - px * ux - py * uy - pz_max * uz;
                ...
                if (INCLUDE_DELTAF) {
                    double Wfactor_max =
                        (pt_max * pt_max * pi00 - 2.0 * pt_max * px * pi01
                         - 2.0 * pt_max * py * pi02
                         - 2.0 * pt_max * pz_max * pi03 + px * px * pi11
                         + 2.0 * px * py * pi12 + 2.0 * px * pz_max * pi13
                         + py * py * pi22 + 2.0 * py * pz_max * pi23
                         + pz_max * pz_max * pi33);
                    delta_f_shear =
                        ((1. - sign * f0_max) * Wfactor_max * deltaf_prefactor);
                }
                ...
                if (INCLUDE_DIFFUSION_DELTAF == 1) {
                    ...
                    delta_f_qmu =
                        ((1. - sign * f0_max)
                         * (prefactor_qmu - baryon / pdotu_max) * qmufactor
                         / deltaf_qmu_coeff);
                }
                ...
                if (ran_gen_ptr->rand_uniform()
                    > results_max / dN_max_sampling) {
                    tries++;
                    continue;
                ...
                for (int sub_try = 0; sub_try < y_minus_eta_tab_length / 2;
                     sub_try++) {
                    // get a y-eta_s index
                    y_minus_eta_s_idx = (floor(
                        ran_gen_ptr->rand_uniform()
                        * (y_minus_eta_tab_length - 1e-30)));
                    double pt =
                        (mT
                         * hypertrig_y_minus_eta_table[y_minus_eta_s_idx][0]);
                    double pz =
                        (mT
                         * hypertrig_y_minus_eta_table[y_minus_eta_s_idx][1]);
            ...
            if (!USE_OSCAR_FORMAT) {
                snprintf(
                    ...
                    "%e  %e  %e  %e\n",
                    FO_idx, surf->tau, surf->xpt, surf->ypt, y_minus_eta_s, pT,
                    phi, surf->da0, surf->da1, surf->da2, surf->u1 / surf->u0,
...
// x-at-the-center-of-the-bin mean-x dN_dx count
{
    ...
    for (long idx = 0; idx < number_of_bins; idx++)
        formatedPrint(
            of, 7, x_min + (idx + 0.5) * dx,
            sum_x1[idx] / (count1[idx] + 1e-30), sum_dN1[idx] / dx, count1[idx],
            sum_x2[idx] / (count2[idx] + 1e-30), sum_dN2[idx] / dx,
            count2[idx]);
    of.close();
}
//***************************************************************************
...
// The output has three columns: phi dN/dphi phi_weight
{
    ...
    for (int j = 0; j < phi_tab_length; j++)
        formatedPrint(
            of, 3, phi_tab->get(1, j + 1), dN_dphi[j], phi_tab->get(2, j + 1));
    of.close();
}
//***************************************************************************
...
// The output has three columns: eta dN/deta eta_weight
{
    ...
    for (int k = 0; k < y_minus_eta_tab_length; k++)
        formatedPrint(
            of, 3, y_minus_eta_tab->get(1, k + 1), dN_deta[k],
        ...
        for (auto const &ev_i : (*Hadron_list)) {
            ...
            for (auto &part_i : (*ev_i)) {
                ...
                gzprintf(
                    ...
                    part_i.mass, part_i.t, part_i.x, part_i.y, part_i.z,
...
void EmissionFunctionArray::calculate_dN_dxtdy_4all_particles() {
    ...
    */
    ...
    for (long l = 0; l < FO_length; l++) {
        ...
        double dsigma_dot_u =
            tau * (da0 * gammaT + ux * da1 + uy * da2 + uz * da3 / tau);
        ...
        if (INCLUDE_DIFFUSION_DELTAF == 1) {
            ...
            dsigma_dot_q =
                tau * (da0 * qmu0 + da1 * qmu1 + da2 * qmu2 + da3 * qmu3 / tau);
        ...
        for (int n = 0; n < number_of_chosen_particles; n++) {
            ...
            if (n > 0 && last_particle_sign == sign
                ...
                && std::abs(
                       ...
                       / (last_particle_mass + 1e-30))
                       < tolerance
                && std::abs(
                       (last_particle_mu - mu) / (last_particle_mu + 1e-30))
                       < tolerance) {
                ...
                if (INCLUDE_DIFFUSION_DELTAF == 1) {
                    deltaN_qmu =
                        (unit_factor * prefactor * dsigma_dot_q
                         / deltaf_qmu_coeff
    ...
    sw.toc();
    cout << endl
         << " -- Calculate_dN_dxtdy_4all_particles finished in "
         << sw.takeTime() << " seconds." << endl;
}
    ...
    for (long l = 0; l < FO_length; l++) {
        ...
        if (INCLUDE_BULK_DELTAF == 1) {
            deltaN_bulk =
                (unit_factor * prefactor * dsigma_dot_u
                 * (-bulkPi * bulkvisCoefficients[0])
                 * (-bulkvisCoefficients[1] * results_ptr[1] + results_ptr[2]));
        }
        ...
        if (INCLUDE_DIFFUSION_DELTAF == 1) {
            deltaN_qmu =
                 ...
                 * (-prefactor_qmu * results_ptr[3] - baryon * results_ptr[4]));
        }
    ...
    */
    ...
    for (int n = 1; n <= truncate_order; n++) {
        ...
        if (INCLUDE_BULK_DELTAF == 1 && bulk_deltaf_kind == 1) {
            ...
            deltaN_bulk_term1 +=
                theta * fugacity * (mass * beta * K_1 + 3. / n * K_2);
...
double EmissionFunctionArray::calculate_total_FZ_energy_flux() {
    ...
    */
    ...
    for (auto const &surf_i : FOsurf_ptr) {
        ...
        total_energy +=
            ((e * u0 * u0 + pi00) * da0 + ((e + p) * u0 * u1 + pi01) * da1
             + ((e + p) * u0 * u2 + pi02) * da2
             + ((e + p) * u0 * u3 + pi03) * da3);
    }
...
//***************************************************************************
void
EmissionFunctionArray::sample_using_dN_dxtdy_4all_particles_conventional() {
    /*
    ...
    */
    ...
    for (int n = 0; n < number_of_chosen_particles; n++) {
        ...
        if (flag_output_samples_into_files == 1) {
            of_control.open(
                samples_control_filename.str().c_str(),
        ...
        for (long repeated_sampling_idx = 1;
            ...
            while (sample_idx <= number_to_sample) {
                ...
                const double maximum_guess = estimate_maximum(
                    surf, real_particle_idx, mass, sign, degen, baryon, strange,
                    charge, z_exp_m_z, bulkvisCoefficients, deltaf_qmu_coeff);

                // next sample pt and phi
                double pT, phi, y_minus_eta_s;
                int status = sample_momemtum_from_a_fluid_cell(
                    mass, degen, sign, baryon, strange, charge, pT_to,
                    y_minus_eta_s_range, maximum_guess, surf,
                    bulkvisCoefficients, deltaf_qmu_coeff, pT, phi,
                    y_minus_eta_s);
                ...
                std::string particle_string = add_one_sampled_particle(
                    repeated_sampling_idx, FO_idx, surf, particle->monval, mass,
                    pT, phi, y_minus_eta_s, eta_s);
                ...
                if (local_charge_conservation == 1 && particle->charge > 0) {
                    ...
                    do {
                        status2 = sample_momemtum_from_a_fluid_cell(
                            ...
                            pT_to, y_minus_eta_s_range, maximum_guess, surf,
                            bulkvisCoefficients, deltaf_qmu_coeff, pT2, phi2,
                            y_minus_eta_s2);
                    } while (status2 == 0);
                    std::string particle_string2 = add_one_sampled_particle(
                        repeated_sampling_idx, FO_idx, surf, -particle->monval,
        ...
        if (flag_output_samples_into_files == 1 && sample_writing_signal != 0) {
            of_sample << sample_str_buffer.str();
            sample_str_buffer.str("");
            sample_writing_signal = 0;
            of_sample.close();
...
void EmissionFunctionArray::initialize_special_function_arrays() {
    ...
    int lambert_tb_length =
        static_cast<int>(((lambert_x_max - lambert_x_min) / lambert_dx) + 1);
    ...
    if (arg < lambert_x_min || arg > lambert_x_max - lambert_dx) {
        if (AMOUNT_OF_OUTPUT > 5) {
            ...
            cout << "lambert_x_min = " << lambert_x_min
                 << ", lambert_x_max = " << lambert_x_max << ", arg = " << arg
                 << endl;
        }
        ...
        results =
            (1. - fraction) * lambert_W[idx] + fraction * lambert_W[idx + 1];
    }
    return (results);
}
    ...
    if (sign == 1) {  // fermion
        ...
        if (rhs > 0.3678794) {
            ...
            if (Emax < mass) {
                ...
                double guess_ideal1 =
                    Emax / (exp((Emax - mu) * inv_Tdec) + sign);
                double guess_ideal2 = mass * f0_mass;
                guess_ideal =
                    (guess_ideal1 > guess_ideal2 ? guess_ideal1 : guess_ideal2);
            }
        }
    }
    return (guess_ideal);
...
double EmissionFunctionArray::estimate_shear_viscous_maximum(
    ...
    if (sign == 1) {
        ...
        guess_viscous =
            Emax * Emax * Emax / (exp((Emax - mu) * inv_Tdec) + sign);
    } else {
        // boson
        double rhs = A * exp(inv_Tdec * mu - A);
        if (rhs > 0.3678794) {
            ...
            if (Emax < mass) {
                ...
                double guess_viscous1 =
                    (Emax * Emax * Emax / (exp((Emax - mu) * inv_Tdec) + sign));
                double guess_viscous2 = mass * mass * mass * f0_mass;
                guess_viscous =
                    (guess_viscous1 > guess_viscous2 ? guess_viscous1
                                                     : guess_viscous2);
            }
        }
    }
...
double EmissionFunctionArray::estimate_diffusion_maximum(
    int sign, int baryon, double mass, double Tdec, double mu, double f0_mass,
    TableFunction &z_exp_m_z, double prefactor_qmu, double guess_ideal,
    double q_size) {
    double inv_Tdec = 1. / Tdec;
    ...
    if (sign == 1) {
        ...
        if (rhs > 0.3678794) {
            ...
            if (Emax < mass) {
                ...
                guess_G2max =
                    (guess_G2max_temp1 > guess_G2max_temp2 ? guess_G2max_temp1
                                                           : guess_G2max_temp2);
            }
        }
    }
...
double EmissionFunctionArray::get_deltaf_bulk(
    double mass, double pdotu, double bulkPi, double Tdec, int sign, double f0,
...
int EmissionFunctionArray::sample_momemtum_from_a_fluid_cell(
    const double mass, const double degen, const int sign, const int baryon,
    const int strange, const int charge, const double pT_to,
    const double y_minus_eta_s_range, const double maximum_guess,
    const FO_surf *surf, const std::array<double, 3> bulkvisCoefficients,
    ...
    while (tries < maximum_impatience) {
        ...
        phi = 2 * M_PI * ran_gen_ptr->rand_uniform();
        y_minus_eta_s =
            ((1. - 2. * ran_gen_ptr->rand_uniform()) * y_minus_eta_s_range);
        ...
        double pdotu =
            p0 * surf->u0 - px * surf->u1 - py * surf->u2 - p3 * surf->u3;
        double expon = (pdotu - mu) * inv_Tdec;
        double f0 = 1. / (exp(expon) + sign);

        double pdsigma =
            (p0 * surf->da0 + px * surf->da1 + py * surf->da2
             + p3 * surf->da3 / surf->tau);

        // delta f for shear viscosity
        double delta_f_shear = 0.;
        if (INCLUDE_DELTAF == 1) {
            double Wfactor =
                (p0 * p0 * surf->pi00 - 2.0 * p0 * px * surf->pi01
                 - 2.0 * p0 * py * surf->pi02 - 2.0 * p0 * p3 * surf->pi03
                 + px * px * surf->pi11 + 2.0 * px * py * surf->pi12
                 + 2.0 * px * p3 * surf->pi13 + py * py * surf->pi22
                 + 2.0 * py * p3 * surf->pi23 + p3 * p3 * surf->pi33);
        ...
        if (INCLUDE_BULK_DELTAF == 1) {
            delta_f_bulk = get_deltaf_bulk(
                mass, pdotu, surf->bulkPi / hbarC, Tdec, sign, f0,
                bulkvisCoefficients);
        }
        ...
        if (INCLUDE_DIFFUSION_DELTAF == 1) {
            double qmufactor =
                (p0 * surf->qmu0 - px * surf->qmu1 - py * surf->qmu2
                 - p3 * surf->qmu3);
            delta_f_qmu =
                ((1. - sign * f0) * (prefactor_qmu - baryon / pdotu) * qmufactor
                 / deltaf_qmu_coeff);
        }
        ...
        result =
            (prefactor * degen * f0 * pdsigma * surf->tau
             * (1.
                + (delta_f_shear + delta_f_bulk + delta_f_qmu)
                      * resize_factor));

        double accept_prob =
            (result / (actual_adjusted_maximum_factor * maximum_guess));
        if (AMOUNT_OF_OUTPUT > 5) {
            if (accept_prob > accept_prob_max) accept_prob_max = accept_prob;
        }
...
double EmissionFunctionArray::estimate_maximum(
    ...
    const double sign, const double degen, const int baryon, const int strange,
    const int charge, TableFunction &z_exp_m_z,
    const std::array<double, 3> bulkvisCoefficients,
    ...
    const iSS_data::ViscousVec pi = {
        surf->pi00, surf->pi01, surf->pi02, surf->pi03, surf->pi11,
        surf->pi12, surf->pi13, surf->pi22, surf->pi23, surf->pi33};
    ...
    const double u_dot_dsigma =
        surf->tau
        * (surf->u0 * surf->da0 + surf->u1 * surf->da1 + surf->u2 * surf->da2
           + surf->u3 * surf->da3 / surf->tau);
    const double dsigma_sq =
        surf->tau * surf->tau
        * (surf->da0 * surf->da0 - surf->da1 * surf->da1 - surf->da2 * surf->da2
           - surf->da3 * surf->da3 / (surf->tau * surf->tau));
    const double dsigmaT =
        sqrt(std::abs(dsigma_sq - u_dot_dsigma * u_dot_dsigma));
    ...
    // (A= power of E, for ideal case A=2), which gives
    // (beta*E-A)*exp(beta*E-A) = +- A*exp(beta*mu-A) --- (*)
    const double guess_ideal =
        estimate_ideal_maximum(sign, mass, Tdec, mu, f0_mass, z_exp_m_z);
    ...
    if (INCLUDE_BULK_DELTAF == 1) {
        guess_bulk =
            (std::abs(bulkPi * bulkvisCoefficients[0]) * mass * mass * inv_Tdec
             / 3. * f0_mass * (1. - sign * f0_mass));
    }
    ...
    if (INCLUDE_DIFFUSION_DELTAF == 1) {
        double qmu_sq =
            (qmu[0] * qmu[0] - qmu[1] * qmu[1] - qmu[2] * qmu[2]
             - qmu[3] * qmu[3]);
        double qmu_mag_over_kappa_hat =
            (sqrt(std::abs(qmu_sq)) / deltaf_qmu_coeff);
        guess_qmu = estimate_diffusion_maximum(
            sign, baryon, mass, Tdec, mu, f0_mass, z_exp_m_z, prefactor_qmu,
            guess_ideal, qmu_mag_over_kappa_hat);
    }

    // combine
    double maximum_guess =
        (prefactor * degen * dsigma_all
         * (guess_ideal + guess_viscous + guess_bulk + guess_qmu));

    return (maximum_guess);
}

std::string EmissionFunctionArray::add_one_sampled_particle(
    const int repeated_sampling_idx, const unsigned long FO_idx,
    const FO_surf *surf, const int particle_monval, const double mass,
    ...
    if (!USE_OSCAR_FORMAT) {
        snprintf(
            line_buffer, 500,
            "%lu  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  %e  "
            "%e  %e\n",
            FO_idx, surf->tau, surf->xpt, surf->ypt, y_minus_eta_s, pT, phi,
            surf->da0, surf->da1, surf->da2, surf->u1 / surf->u0,
            surf->u2 / surf->u0, rapidity_y, eta_s, E, p_z, t, z);
    } else {
        // To be combined to OSCAR
        snprintf(
            line_buffer, 500,
