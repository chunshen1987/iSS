...
iSS::iSS(
    std::string path, std::string table_path, std::string particle_table_path,
    std::string inputfile, std::string surface_filename)
    : path_(path),
...
int iSS::read_in_FO_surface() {
    ...
    read_FOdata freeze_out_data(
        paraRdr_ptr, path_, table_path_, particle_table_path_);
...
int iSS::generate_samples() {
    ...
    if (afterburner_type_ == AfterburnerType::SMASH) {
        ...
        chosen_particles.loadTableFromFile(
            particle_table_path_ + "/chosen_particles_s95p-v1.dat");
    }

    messager.info("Start computation and generating samples ...");
    if (paraRdr_ptr->getVal("MC_sampling") == 4) {
        spectra_sampler_ = std::unique_ptr<FSSW>(new FSSW(
            ran_gen_ptr_, &chosen_particles, particle_, FOsurf_LRF_array_,
            flag_PCE_, paraRdr_ptr, path_, table_path_, afterburner_type_));
        ...
        efa_ = std::unique_ptr<EmissionFunctionArray>(new EmissionFunctionArray(
            ...
            particle_, FOsurf_array_, flag_PCE_, paraRdr_ptr, path_,
            table_path_, afterburner_type_));
        efa_->shell();
    }
    return (0);
}
...
void iSS::transform_to_local_rest_frame(
    ...
    for (auto &surf_i : FOsurf_ptr) {
        ...
        Vec4 qmu_tz = {
            surf_i.qmu0 * cosh_eta + surf_i.qmu3 * sinh_eta, surf_i.qmu1,
            surf_i.qmu2, surf_i.qmu3 * cosh_eta + surf_i.qmu0 * sinh_eta};
...
void iSS::compute_spin_polarization() {
    polarizor = std::unique_ptr<SpinPolarization>(new SpinPolarization(
        FOsurf_array_, particle_, path_, table_path_, *paraRdr_ptr));
...
void iSS::construct_Tmunu_from_particle_samples() {
    ...
    double volume =
        (FOsurf_LRF_array_[0].da_mu_LRF[0] / FOsurf_LRF_array_[0].u_tz[0]);
    int monval_cached = 0;
    ...
    for (int i = 0; i < 3; i++) netChargeArr[i] /= (nev * volume);
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            output << std::scientific << std::setprecision(8)
                   ...
                   << FOsurf_Tmunu_[4 * i + j] - T[i][j] << std::endl;
            messager << "check: T[" << i << "][" << j
                     << "] = " << FOsurf_Tmunu_[4 * i + j] << " GeV/fm^3,  "
                     << T[i][j] << " GeV/fm^3, diff = "
                     << FOsurf_Tmunu_[4 * i + j] - T[i][j] << " GeV/fm^3";
            messager.flush("info");
        }
    }
    for (int i = 0; i < 3; i++) {
        if (i == 0) {  // only check net baryon density for now
            output << std::scientific << std::setprecision(8) << FOsurf_Q_[i]
                   << "  " << netChargeArr[i] << "  "
                   << FOsurf_Q_[i] - netChargeArr[i] << std::endl;
        }
        messager << "check: nQ[" << i << "] = " << FOsurf_Q_[i] << " 1/fm^3,"
                 << netChargeArr[i]
                 << " 1/fm^3, diff = " << FOsurf_Q_[i] - netChargeArr[i]
                 << " 1/fm^3";
        messager.flush("info");
    }
    output.close();
...
void iSS::computeFOSurfTmunu(std::vector<FO_surf> &FOsurf_ptr) {
    ...
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++) gmunu[i][j] = 0.;
    gmunu[0][0] = 1.;
    for (int i = 1; i < 4; i++) gmunu[i][i] = -1.;
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            messager << "T[" << i << "][" << j << "] = " << std::scientific
                     << std::setprecision(6) << FOsurf_Tmunu_[4 * i + j]
                     << " GeV/fm^3.";
            messager.flush("info");
        }
    }
