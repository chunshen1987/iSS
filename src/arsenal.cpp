...
//**********************************************************************
double sixPoint2dInterp(
    double x, double y, double v00, double v01, double v02, double v10,
...
// -- x,y: the independent and dependent double varables; x is assumed to be
    ...
    if (!allowExtrapolation) {
        ...
        if (idx == 0) {
            ...
            double A0 = (*y)[0], A1 = (*y)[1], A2 = (*y)[2],
                   deltaX =
                       xx - (*x)[0];  // deltaX is the increment of x0 compared
            ...
            double A0 = (*y)[size - 3], A1 = (*y)[size - 2],
                   A2 = (*y)[size - 1],
                   deltaX = xx - ((*x)[0] + (idx - 1) * dx);
            return (A0 - 2.0 * A1 + A2) / (2.0 * dx * dx) * deltaX * deltaX
                   - (3.0 * A0 - 4.0 * A1 + A2) / (2.0 * dx) * deltaX + A0;
        }
    } else {
...
double interpCubicMono(
    std::vector<double>* x, std::vector<double>* y, double xx,
    bool allowExtrapolation)
// Note that this function does NOT perform well with small x and y table
// spacing; in which case use "direct" version instead. Returns the interpreted
// value of y=y(x) at x=x0 using cubic polynomial interpolation method.
...
// -- xx: where the interpolation should be performed
{
    ...
    C0 = (x0 * x02 * x2 * x03 * x23 * x3 * y1
          ...
          + x1s
                * (x0 * x3 * (-x0s + x3s) * y2 + x2 * x2s * (x3 * y0 - x0 * y3)
    ...
    C1 = (x0s * x03 * x3s * y12 + x2 * x2s * (x3s * y01 + x0s * y13)
         ...
         / denominator;
    C2 = (-x0 * x3 * (x0s - x3s) * y12 + x2 * (x3 * x3s * y01 + x0 * x0s * y13)
...
double adaptiveSimpsonsAux(
    double (*f)(double), double a, double b, double epsilon, double S,
    ...
    return adaptiveSimpsonsAux(
               f, a, c, epsilon / 2, Sleft, fa, fc, fd, bottom - 1)
           + adaptiveSimpsonsAux(
               f, c, b, epsilon / 2, Sright, fc, fb, fe, bottom - 1);
}
//
// Adaptive Simpson's Rule
//
double adaptiveSimpsons(
    double (*f)(double),      // ptr to function
    ...
    return adaptiveSimpsonsAux(
        f, a, b, epsilon, S, fa, fb, fc, maxRecursionDepth);
}
...
{
    double f_1 = f(a) + f(b), f_2 = 0.,
           f_4 = 0.;  // sum of values of f(x) that will be weighted by 1, 2, 4
                      // respectively, depending on where x is
    double sum_previous = 0.,
           sum_current = 0.;  // previous and current sum (intgrated value)

    long count = 1;           // how many new mid-points are there
    double length = (b - a),  // helf length of the interval
        step =
            length
            / count;  // mid-points are located at a+(i+0.5)*step, i=0..count-1

    int currentRecursionDepth = 1;

    f_4 = f(a + 0.5 * step);  // mid point of [a,b]
    sum_current =
        (length / 6) * (f_1 + f_2 * 2. + f_4 * 4.);  // get the current sum

    do {
        sum_previous = sum_current;  // record the old sum
        f_2 += f_4;  // old mid-points with weight 4 will be new mid-points with
                     // weight 2
        ...
        for (int i = 0; i < count; i++)
            f_4 += f(a + step * (i + 0.5));  // sum up f_4

        sum_current = (length / 6 / count)
                      * (f_1 + f_2 * 2. + f_4 * 4.);  // calculate current sum
        // cout << sum_current << endl;

        if (currentRecursionDepth > maxRecursionDepth) {
            cout << endl
                 << "Warning qiu_simpsons: maximum recursion depth reached!"
                 << endl
                 << endl;
...
// the upper table range.
{
    ...
    if (value > (*A)[idx_f]) {
        if (skip_out_of_range)
            return length - 1;  // normal return value is between 0 and length-2
        cout << "binarySearch: desired value is too large, exceeding the end "
                "of the table."
             << endl;
        exit(-1);
    }
    if (value < (*A)[idx_i]) {
        if (skip_out_of_range)
            return -1;  // normal return value is between 0 and length-2
        cout << "binarySearch: desired value is too small, exceeding the "
                "beginning of table."
             << endl;
        exit(-1);
    }
...
{
    if (x <= 0 || x > XBIG) return HUGE_VAL;

    if (x <= MACHINE_EPSILON) return -log(x);

    if (x <= 4) {
        double p1[8] = {4.945235359296727046734888E+00,
               ...
               q1[8] = {6.748212550303777196073036E+01,
               ...
               p2[8] = {4.974607845568932035012064E+00,
               ...
               q2[8] = {1.830328399370592604055942E+02,
        ...
        double xden = 1, xnum = 0,
               xm = x <= 1.5 ? (x > 0.5 ? x - 1 : x) : x - 2;
        bool flag = false;
        ...
        return (x > 1.5 ? 0 : corr)
               + xm * ((flag ? D1 : D2) + xm * (xnum / xden));
    }

    if (x <= 12) {
        double xm = x - 4, xden = -1, xnum = 0,
               p[8] = {1.474502166059939948905062E+04,
               ...
               q[8] = {2.690530175870899333379843E+03,
...
{
    return exp(
        log_gamma_function(x) + log_gamma_function(y)
        - log_gamma_function(x + y));
}
...
// Print out a progress bar with the given percentage. Use a negative value to
...
// For easier scientific data outputing.
{
    va_list ap;
    va_start(
        ap, count);  // Requires the last fixed parameter (to get the address)
    for (int j = 0; j < count; j++)
        os << scientific << setprecision(OUTPUT_PRECISION) << "  "
           << va_arg(ap, double);  // Requires the type to cast to. Increments
                                   // ap to the next argument.
    va_end(ap);
    os << endl;
}
...
// Personal amusement.
{
    switch (which) {
        ...
        case 3:
            cout << " __  __     ______     __     __   __     _____    "
                 << endl;
            cout << "/\\ \\_\\ \\   /\\  ___\\   /\\ \\   /\\ '-.\\ \\   "
                    "/\\___  \\  "
                 << endl;
...
//**********************************************************************
void GaussLegendre_getWeight(
    int npts, double* xg, double* wg, double A, double B, int iop)
...
//**********************************************************************
void get_bin_average_and_count(
    istream& is, ostream& os, vector<double>* bins, long col_to_bin,
    void (*func)(vector<double>*), long wanted_data_columns, bool silence)
...
// Refer also to getBinnedAverageAndCount MATLAB program.
{
    ...
    if (number_of_cols == 0) {
        cout << "get_bin_average_and_count error: the input data is empty!"
             << endl;
        exit(-1);
    }
    ...
    while (is.eof() == false) {
        ...
        if (bin_idx != -1) {
            ...
            for (long j = 0; j < number_of_cols; j++)
                bin_total_and_count[bin_idx][j] += line_data[j];
        ...
        if (number_of_lines % 100000 == 0 && !silence)
            cout << "Line " << number_of_lines << " reached." << endl;
        number_of_lines++;
    }
    ...
    for (long i = 0; i < number_of_bins; i++) {
        if (bin_total_and_count[i][number_of_cols] < 1e-15) continue;
        bin_total_and_count[i][number_of_cols + 1] =
            bin_total_and_count[i][number_of_cols]
            / ((*bins)[i + 1] - (*bins)[i]);
    }
    ...
    for (long i = 0; i < number_of_bins; i++) {
        for (long j = 0; j < number_of_cols + 2; j++) {
            os << scientific << scientific << setprecision(OUTPUT_PRECISION)
               << bin_total_and_count[i][j] << "  ";
        }
        os << endl;
    }
}

double*** create_a_3D_Matrix(
    const int nx, const int ny, const int nz, const double val) {
...
void set_val_in_3D_Matrix(
    double*** Arr_3D, const int nx, const int ny, const int nz,
    const double val) {
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            for (int k = 0; k < nz; k++) {
...
double**** create_a_4D_Matrix(
    const int nx, const int ny, const int nz, const int n4, const double val) {
...
void delete_a_4D_Matrix(
    double**** Arr_4D, const int nx, const int ny, const int nz) {
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            for (int k = 0; k < nz; k++) {
                delete[] Arr_4D[i][j][k];
...
void set_val_in_4D_Matrix(
    double**** Arr_4D, const int nx, const int ny, const int nz, const int n4,
    const double val) {
    for (int i = 0; i < nx; i++) {
        for (int j = 0; j < ny; j++) {
            for (int k = 0; k < nz; k++) {
