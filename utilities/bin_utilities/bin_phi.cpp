...
{
  ofstream vn_format("results/bin_phi_format.dat");
  for (int order=1; order<=calculate_vn_to_order; order++)
  {
  ...
  vn_format << "dN_dphi = " << total_number_of_columns+calculate_vn_to_order+2 << endl;
  vn_format.close();
}
...
int main()
{
  ...
  Table bins("tables/phi_bins.dat");
  ...
  for (int i=1; i<=chosen_particles.getNumberOfRows(); i++)
  {
    ...
    sprintf(out_filename_buffer, "results/samples_%d_bin_phi.dat", (int) chosen_particles.get(1,i));
    ifstream in_f(in_filename_buffer);
    ofstream out_f(out_filename_buffer);
    get_bin_average_and_count(in_f, out_f, bins.getColumn(1), phi_col_idx, operation, total_number_of_columns+calculate_vn_to_order);
  }
