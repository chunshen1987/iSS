...
{
  ofstream vn_format("results/bin_eta_format.dat");
  for (int order=1; order<=calculate_vn_to_order; order++)
  {
  ...
  vn_format << "dN_deta = " << total_number_of_columns+calculate_vn_to_order+2 << endl;
  vn_format.close();
}
...
int main()
{
  ...
  Table bins("tables/eta_bins.dat");
  ...
  int eta_col_idx = format.getVal("eta_s");
  ...
  for (int i=1; i<=chosen_particles.getNumberOfRows(); i++)
  {
    ...
    sprintf(out_filename_buffer, "results/samples_%d_bin_eta.dat", (int) chosen_particles.get(1,i));
    ifstream in_f(in_filename_buffer);
    ofstream out_f(out_filename_buffer);
    get_bin_average_and_count(in_f, out_f, bins.getColumn(1), eta_col_idx, operation, total_number_of_columns+calculate_vn_to_order);
  }
