...
int main()
{
  ...
  vector<double> bins(2); bins[0] = 0; bins[1] = 99.0;
  ...
  for (int i=1; i<=chosen_particles.getNumberOfRows(); i++)
  {
    ...
    get_bin_average_and_count(in_f, out_f, &bins, pT_col_idx, operation, total_number_of_columns+calculate_vn_to_order*2);
  }
